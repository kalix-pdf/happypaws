...
class ControllerExtensionModuleMarketplace extends Controller
{
	...
	public function index()
	{
		...
		if (isset($this->request->get['route']) and (substr($this->request->get['route'], 0, 8) == 'account/')) {
			...
			$data['account_menu_href'] = array(
				...
				'customers' => $this->url->link('account/customerpartner/customers', '', true),
				'sales' => $this->url->link('account/customerpartner/sales', '', true),
			);
			...
			if (isset($id['id']) and $id['id']) {
				...
				if ($partner) {
					...
					} else if ($partner['avatar'] && file_exists(DIR_IMAGE . $partner['avatar'])) {
					...
					$partner['sellerHref'] = $this->url->link('customerpartner/profile&seller_id=' . $id['id'] . '&tab=profile', '', true);
				...
				$data['feedback_total'] = $this->model_customerpartner_master->getAverageFeedback($partner['customer_id']);
				$data['writeFeedback'] = $this->url->link('customerpartner/profile/writeFeedback&id=' . $partner['customer_id'], '', true);
				...
				if ($this->config->get('marketplace_review_only_order')) {
					...
					if ($this->customer->isLogged() && ($partner['customer_id'] != $this->customer->getId())) {
						$check_customer = $this->model_customerpartner_master->checkCustomerBought($partner['customer_id']);
				...
				$data['achivment'] = $this->model_customerpartner_achivment->getAchivment($partner['customer_id']);
	...
	public function loadLocation()
	{
		if ($this->request->get['location']) {
			...
			$location = '<iframe id="seller-location" width="100%" height="200" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?q=india&output=embed&z=15"></iframe>';
	...
	public function sellmenu()
	{
		...
		if (isset($this->request->get['route']) && $this->request->get['route'] == 'account/customerpartner/become_partner') {
	...
	public function registerseller()
	{
		...
		if ($this->config->get('module_marketplace_status') && $this->config->get('marketplace_becomepartnerregistration')) {
			...
			/*
				...
				*/
			if (isset($this->request->post['shoppartner']) &&  isset($this->request->post['tobecomepartner']) && $this->request->post['tobecomepartner'] == 1 && !preg_replace('/\s+/', '', $this->request->post['shoppartner'])) {
		...
		if ($this->config->get('module_marketplace_status')) {
			...
			if ($this->config->get('marketplace_seller_info_by_module') && !$this->config->get('marketplace_seller_info_hide')) {
				...
				if ($check_seller) {
					...
					$data['loadProfile'] = $this->url->link('customerpartner/profile&seller_id=' . $check_seller['customer_id'], '', true);
