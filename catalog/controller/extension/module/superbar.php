...
class ControllerExtensionModuleSuperbar extends Controller
{
	public function index()
	{
		if ($this->config->get('module_marketplace_status')) {
			...
			$data['superbar_status'] = false; //$this->config->get('module_superbar_status');
			...
			$data['superbar_gift_tooltip'] = !empty($this->config->get('superbar_gift_tooltip')) ? $this->config->get('superbar_gift_tooltip')[$language_id] : '';
			...
			$data['gift_title'] = !empty($this->config->get('wk_gwidget_title')) ? $this->config->get('wk_gwidget_title')[$language_id] : '';
			$data['upsell_status'] =  false;//$this->config->get('module_wk_upsell_upsell_status');
			$data['crosssell_status'] = false;//$this->config->get('module_wk_crosssell_crosssell_status');
			...
			$data['hposition'] = $this->config->get('superbar_position_hori') ? 'top' : 'bottom';
			$data['hper'] = $this->config->get('superbar_hori');
			$data['vposition'] = $this->config->get('superbar_position_verti') ? 'left' : 'right';
			...
			$data['uheader2'] = addslashes(html_entity_decode(!empty($this->config->get('wk_widget_upselling_details2')) ? $this->config->get('wk_widget_upselling_details2')[$language_id] : '')); //
			$data['cheader1'] = addslashes(html_entity_decode($this->config->get('wk_cwidget_crossselling_details')[$language_id]));
			$data['cheader2'] = addslashes(html_entity_decode(!empty($this->config->get('wk_cwidget_crossselling_details2')) ? $this->config->get('wk_cwidget_crossselling_details2')[$language_id] : ''));
			$data['gheader1'] = addslashes(html_entity_decode(!empty($this->config->get('wk_gwidget_gifting_details')) ? $this->config->get('wk_gwidget_gifting_details')[$language_id] : ''));
			$data['gheader2'] = addslashes(html_entity_decode(!empty($this->config->get('wk_gwidget_gifting_details2')) ? $this->config->get('wk_gwidget_gifting_details2')[$language_id] : ''));
