...
class ControllerExtensionDQuickcheckoutShippingAddress extends Controller {
    ...
    */
    private function getDefault($populate = true){
        ...
        $state = $this->model_extension_d_quickcheckout_store->getState();
        if($populate){
            if (!empty($state['session']['shipping_address']) && is_array($state['session']['addresses']) && array_key_exists($state['session']['shipping_address']['address_id'], $state['session']['addresses']) && !$clear_session) {
