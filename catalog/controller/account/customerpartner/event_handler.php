...
class ControllerAccountCustomerpartnerEventHandler extends Controller
{
  ...
  public function handleCartUpdate(&$path, &$filter_data)
  {
    ...
    if (!empty($this->request->post['quantity'])) {
      ...
      $quantity_restriction = $this->config->get('marketplace_product_purchase_limit_qra_seller');
      $price_restriction = $this->config->get('marketplace_product_purchase_limit_pra_seller');
      $priority_restriction = $this->config->get('marketplace_product_purchase_limit_priority');
        ...
        if (!empty($seller_restriction) && $this->flag) {
          if (isset($seller_restriction['price_status']) && $seller_restriction['price_status'] && $priority_restriction) {
          ...
          if (isset($seller_restriction['quant_status']) && $seller_restriction['quant_status'] && !$priority_restriction) {
      ...
      if(!(isset($seller_restriction['price_status']) && $seller_restriction['price_status']) || !(isset($seller_restriction['quant']) && $seller_restriction['quant'])){
        if($quantity_restriction == 1 && $price_restriction == 1 && ($priority_restriction == 0 || $priority_restriction == 1)) {
          if($priority_restriction == 1 ){
            $this->price_status = true;
            $this->price_value = $this->config->get('marketplace_product_purchase_price_limit');
          }else{
            $this->quant_status = true;
            $this->quant_value = $this->config->get('marketplace_product_purchase_limit');
          }
        }
      }
      ...
      if($quantity_restriction == 1 && $price_restriction == 1 && ($priority_restriction == 0 || $priority_restriction == 1)) {
        if (($this->price_status && $this->price_value) || ($this->quant_status && $this->quant_value)) {
          ...
          if ($this->price_status && $this->price_value && $this->seller_id) {
            ...
            foreach ($this->cart->getProducts() as $key => $value) {
              foreach ($this->cart_products[$this->seller_id] as $seller_key => $seller_value) {
                if($seller_key == $value['product_id']){     
                  if (isset($value['cart_id']) && isset($this->request->post['quantity'][$value['cart_id']])) {
                    ...
                    $total = $price_q;
                    if (((float)$total > (float)$this->config_limit_value) && $this->config->get('marketplace_product_purchase_limit_pra_seller')) {
                      $this->session->data['success'] =  '<b style="color:red;"> This product can not be added to the cart as there is limit for the total cart value of the Seller Product  </b><b style="color:black;"> You can not add product having price more than  ' . $this->config_limit_value . ' of total cart value </b>';
            ...
            switch ($this->config_limit_type) {
              case 1: // Based on the Total number of the product of the Single Seller
                ...
                if (!empty($this->cart_products) && ($total > $this->config_limit_value)) {
                  $this->session->data['success'] = '<b style="background-color:red;color:black;"> This product can not be added to the cart as there is limit for the Seller`s Product Quantity</b><b style="color:black;"> You can not add more than ' . $this->config_limit_value . ' product into the cart for the same seller</b>';
                  $this->request->post['quantity'] =  array();
              ...
              case 0: // Based on the Quantity of the single product
                ...
                if ($this->seller_id) {
                  ...
                  if ($total > $this->config_limit_value) {
                    $this->session->data['success'] = '<b style="color:red;"> This product can not be added to the cart as there is limit for the number of the products Sellers Product  </b><b style="color:black;"> You can not add more than ' . $this->config_limit_value . ' quantity </b>';
                    $this->request->post['quantity'] =  array();
                    $this->response->redirect($this->url->link('checkout/cart'));
                  }
                }
        ...
        if ($this->flag && $this->config->get('marketplace_product_purchase_limit') && $this->config->get('marketplace_product_purchase_limit_qra_seller')) {
          ...
          switch ($this->config_limit_type) {
            case 1: // Based on the Total number of the product of the Single Seller
              ...
              if (!empty($this->cart_products) && ($total > $this->config_limit_value)) {
                $this->session->data['success'] = '<b style="color:red;"> This product can not be added to the cart as there is limit for the Seller`s Product Quantity</b><b style="color:black;"> You can not add more than ' . $this->config_limit_value . ' product into the cart for the same seller</b>';
                $this->request->post['quantity'] =  array();
                $this->response->redirect($this->url->link('checkout/cart'));
              }
    ...
    if($quantity_restriction == 1 && $price_restriction == 1 && ($priority_restriction == 0 || $priority_restriction == 1)) {
      if (($this->price_status && $this->price_value) || ($this->quant_status && $this->quant_value)) {
        ...
        if ($this->price_status && $this->price_value && $this->seller_id) {
          ...
          if ($total > $this->config_limit_value && $this->config->get('marketplace_product_purchase_limit_pra_seller')) {
            $this->session->data['error_order_limit'] = '<b style="color:red;"> This product can not be added to the cart as there is limit for the total cart value of the Seller Product  </b><b style="color:black;"> You can not add product having price more than  ' . $this->config_limit_value . ' of total cart value </b>';
        ...
        if ($this->quant_status && $this->quant_value && $this->seller_id) {
          ...
          switch ($this->config->get('marketplace_product_purchase_limit_based_on')) {
            ...
            case 0: // Based on the Quantity of the single product
              ...
              if ($total > $this->config_limit_value) {
                $this->session->data['error_order_limit'] = '<b style="color:red;"> This product can not be added to the cart as there is limit for the quantity of the Seller Product  </b><b style="color:black;"> You can not add more than ' . $this->config_limit_value . ' quantity </b>';
                $this->session->data['common_cart_total'] = $this->getCommonCartTotal();
              }
              $this->flag = false;
              break;
  ...
  public function getSellerRestriction()
  {
    ...
    $this->seller_id = $this->ocutilities->getProductSellerID($this->request_product_id);
    return $this->model_account_customerpartner_restriction->getRestrictions($this->seller_id);
  }
