...
class ControllerAccountCustomerpartnerReview extends Controller
{
	...
	protected function getList($data)
	{
		...
		foreach ($results as $result) {
			...
			$data['reviews'][] = array(
				...
				'approve'			=> ($result['status']) ? 0 : $this->url->link('account/customerpartner/review/approve', 'review_id=' . $result['id'] . $url, true)
			);
		}
	...
	public function approve()
	{
		...
		if (!empty($this->request->get['review_id']) && $this->request->get['review_id']) {

			if ($this->model_customerpartner_review->approvReview($this->request->get['review_id'])) {
				$this->session->data['success'] = $this->language->get('text_success_review');
			} else {
				$this->session->data['success'] = $this->language->get('text_error_review');
			}
		}
		$url = '';
