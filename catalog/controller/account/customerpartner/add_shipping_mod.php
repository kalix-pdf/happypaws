...
class ControllerAccountCustomerpartneraddshippingmod extends Controller
{
	...
	public function index()
	{
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$data['result_shipping'][] = array(
					...
					'id' => $result['id'],
					'price' => $result['price'],
					'country' => ($result['country_code'] == '*') ? 'All' : $result['country_code'],
					'zip_to' => ($result['zip_to'] == '*') ? 'All' : $result['zip_to'],
					'zip_from' => ($result['zip_from'] == '*') ? 'All' : $result['zip_from'],
					'city' => ($result['city'] == '*') ? 'All' : $result['city'],
		...
		$data['sort_city'] = $this->url->link('account/customerpartner/add_shipping_mod', '&sort=cs.city' . $url, true);
	...
	public function add()
	{
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_account_add_shipping_mod->deletEeventShipping($this->customer->getId());
	...
	public function stepTwo($fields = array())
	{
		...
		// shipping data
		$data['shippingTable'] = array('country_code', 'zip_to', 'zip_from', 'city', 'price', 'weight_to', 'weight_from', 'max_days');
