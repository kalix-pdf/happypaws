	...
	public function getlist()
	{
		...
		foreach ($filter_array as $unsetKey => $key) {
			...
			if (isset($this->request->get[$key])) {
				...
				elseif ($key == 'start')
					$filter_array[$key] = ($filter_array['page'] - 1) * $limit;
				elseif ($key == 'limit')
					$filter_array[$key] = $limit;
				else
		...
		$data['paid'] = $this->currency->format($this->currency->convert($paid, $this->config->get('config_currency'), $this->session->data['currency']), $this->session->data['currency']);
		...
		$data['results'] = sprintf($this->language->get('text_pagination'), ($product_total) ? (($filter_array['page'] - 1) * $limit) + 1 : 0, ((($filter_array['page'] - 1) * $limit) > ($product_total - $limit)) ? $product_total : ((($filter_array['page'] - 1) * $limit) + $limit), $product_total, ceil($product_total / $limit));
		...
		$data['sales_link'] = $this->url->link('account/customerpartner/sales', '', true);
