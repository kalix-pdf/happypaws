...
class ControllerAccountCustomerpartnerNotification extends Controller
{
	...
	public function index()
	{
		...
		if ($seller_notifications) {
			foreach ($seller_notifications as $key => $seller_notification) {
				...
				if ($seller_notification['key'] == 'order_account') {
					...
					if (is_array($seller_notification['data']) && isset($seller_notification['data']['order_id'])) {
						$data['seller_notifications'][] = sprintf(
							$this->language->get('text_order_add'),
							$this->url->link('account/customerpartner/orderinfo&order_id=' . $seller_notification['data']['order_id'], '', true),
							$seller_notification['data']['order_id'],
							$seller_notification['data']['name'],
							$seller_notification['date_added']
						);
					} else {
						// Handle missing 'order_id' gracefully
						$data['seller_notifications'][] = 'Invalid order data received.';
					}
					...
					$data['seller_notifications'][] = sprintf($this->language->get('text_order_return'), $seller_notification['data']['name'], $this->url->link('account/customerpartner/orderinfo&order_id=' . $order_id['order_id'], '', true), $seller_notification['data']['return_id'], $order_id['product'], $seller_notification['date_added']);
					...
					if ($status) {
						if (empty($data['selected']) || in_array('all', $data['selected'])) {
							$data['seller_notifications'][] = sprintf($this->language->get('text_order_status'), $this->url->link('account/customerpartner/orderinfo&order_id=' . $seller_notification['data']['order_id'], '', true), $seller_notification['data']['order_id'], $status['name'], $seller_notification['date_added']);
						} else {
		...
		if ($seller_product_reviews) {
			foreach ($seller_product_reviews as $key => $seller_product_review) {
				...
				if ($seller_product_review['key'] == 'product_review') {
					$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_review'), $seller_product_review['id'], $seller_product_review['data']['author'], $this->url->link('product/product&product_id=' . $seller_product_review['data']['product_id'], '', true), $seller_product_review['data']['product_name'], $seller_product_review['date_added']);
				} elseif ($seller_product_review['key'] == 'product_stock') {
					$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_stock'), $this->url->link('product/product&product_id=' . $seller_product_review['data']['product_id'], '', true), $seller_product_review['data']['product_name'], $seller_product_review['date_added']);
				} elseif ($seller_product_review['key'] == 'product_approve') {
					$data['seller_product_reviews'][] = sprintf($this->language->get('text_product_approve'), $this->url->link('product/product&product_id=' . $seller_product_review['data']['product_id'], '', true), $seller_product_review['data']['product_name'], $seller_product_review['date_added']);
				}
			}
		}
		...
		if ($seller_reviews) {
			foreach ($seller_reviews as $key => $seller_review) {
				...
				$data['seller_reviews'][] = sprintf($this->language->get('text_seller_review'), $seller_review['id'], $this->url->link('customerpartner/profile&seller_id=' . $seller_review['customer_id'], '', true), $seller_review['name'], $seller_review['createdate']);
			}
		}
		...
		if ($categories) {
			foreach ($categories as $key => $category) {
				...
				if (isset($category['data']['category_name']) && $category['data']['category_name']) {
					$data['seller_reviews'][] = sprintf($this->language->get('text_category_approve'), $this->url->link('account/customerpartner/category&filter_name=' . $category['data']['category_name'], '', true), $category['data']['category_name'], $category['date_added']);
				}
			}
		}
	...
	public function notifications()
	{
		if ($this->customer->getId()) {
			...
			if ($seller_notifications) {
				foreach ($seller_notifications as $key => $seller_notification) {
				if (!isset(json_decode($seller_notification['data'],1)['order_id']))
					continue;
					$date_diff = (array)(new DateTime($seller_notification['date_added']))->diff(new DateTime());
					...
					if ($seller_notification['key'] == 'order_account') {
						$seller_notification['data'] = json_decode($seller_notification['data'], 1);
						$data['seller_notifications'][] = sprintf($this->language->get('text_order_add_mp'), $this->url->link('account/customerpartner/orderinfo&order_id=' . $seller_notification['data']['order_id'], '', true), $seller_notification['data']['order_id'], $seller_notification['data']['name'], $seller_notification['date_added']);
					} elseif ($seller_notification['key'] == 'return_account') {
						$seller_notification['data'] = json_decode($seller_notification['data'], 1);
						$order_id = $this->model_account_notification->getReturnOrderId($seller_notification['data']['return_id']);
						$data['seller_notifications'][] = sprintf($this->language->get('text_order_return_mp'), $seller_notification['data']['name'], $this->url->link('account/customerpartner/orderinfo&order_id=' . $order_id['order_id'], '', true), $seller_notification['data']['return_id'], $order_id['product'], $seller_notification['date_added']);
					} elseif ($seller_notification['key'] == 'order_status') {
						$seller_notification['data'] = json_decode($seller_notification['data'], 1);
						$status = $this->model_mp_localisation_order_status->getOrderStatus($seller_notification['data']['status']);
						if ($status) {
							$data['seller_notifications'][] = sprintf($this->language->get('text_order_status_mp'), $this->url->link('account/customerpartner/orderinfo&order_id=' . $seller_notification['data']['order_id'], '', true), $seller_notification['data']['order_id'], $status['name'], $seller_notification['date_added']);
						}
					}
				}
			}
			...
			if ($seller_reviews) {
				foreach ($seller_reviews as $key => $seller_review) {
					if ($seller_review) {
						...
						$data['seller_reviews'][] = sprintf($this->language->get('text_seller_review_mp'), $seller_review['id'], $this->url->link('customerpartner/profile&seller_id=' . $seller_review['customer_id'], '', true), $seller_review['name'], $seller_review['createdate']);
					}
				}
			}
