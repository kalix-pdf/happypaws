...
class ControllerAccountCustomerpartnerAddproduct extends Controller
{
	...
	public function index()
	{
			...
			if (isset($isMember['membershipType']) && $isMember['membershipType'] == 'quantity') {
				...
				foreach ($sellerDetail as $detail) {
					$data['sellerDetail'][] = array(
						...
						'price'    => $this->currency->format((float)$detail['gprice'], $this->session->data['currency']),
					);
				}
		...
		// 	$this->document->addScript('catalog/view/javascript/wk_summernote/_summernote.js');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') and $this->validateForm()) {
			...
			if (isset($this->request->post['subs_ppd']) || isset($this->request->post['subs_ppm']) || isset($this->request->post['subs_cms'])) {		
				...
				if (isset($this->request->post['subs_cms'])) {
					$amount = (float)$this->request->post['subs_cms'];
					$sbstype = 3;
				}
			}
		...
		if (isset($this->request->post['price'])) {
			...
			$data['price'] = $this->currency->convert((float)$product_info['price'], $this->config->get('config_currency'), $this->session->data['currency']);
		...
		foreach ($data['product_discounts'] as $key => $value) {
			if ($value['price']) {
				$data['product_discounts'][$key]['price'] = number_format($this->currency->convert((float)$value['price'], $this->config->get('config_currency'), $this->session->data['currency']), 2, '.', '');
			} else {
				$data['product_discounts'][$key]['price']  = $value['price'];
			}
		}
		...
		foreach ($data['product_specials'] as $key => $value) {
			if ($value['price']) {
				$data['product_specials'][$key]['price'] = number_format($this->currency->convert((float)$value['price'], $this->config->get('config_currency'), $this->session->data['currency']), 2, '.', '');
			} else {
				$data['product_specials'][$key]['price'] = $value['price'];
			}
		}
		...
		if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_account_customerpartner->getProductImages($this->request->get['product_id'], $tabletype);
		} else {
		...
		// membership codes starts here
		if ($this->config->get('module_wk_seller_group_status')) {
			$data['module_wk_seller_group_status'] = $data['wk_seller_group_status'];
			$data['module_wk_seller_group_single_category'] = $data['wk_seller_group_single_category'];
		}
		// membership codes ends here
	...
	protected function validateForm()
	{
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			if (isset($value['name']) && is_string($value['name']) && (utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 255)) {
				...
				// if($this->config->get('marketplace_product_seo_product_name') && preg_match('/-/', $value['name'])) {
				//   $this->error['name'][$language_id] = $this->language->get('error_name_seo');
				// }
			}
		...
		if (!isset($this->error['length']) && !isset($this->error['width']) && !isset($this->error['height'])) {
			$length = (float) $this->request->post['length'];
			$width = (float) $this->request->post['width'];
			$height = (float) $this->request->post['height'];
		
			if ($length <= 0 || $length > 99) {
				$this->error['length'] = "Length must be between 1 and 99 cm!";
			}
		
			if ($width <= 0 || $width > 99) {
				$this->error['width'] = "Width must be between 1 and 99 cm!";
			}
		...
		$sbstype = 0;
		$valid_days = [];
		if (!empty($this->request->post['subscription'])) 
		{
			if (isset($this->request->post['subs_cms'])) {
				$sbstype = 3;
			}
			...
			if (($sbstype == 1 || $sbstype == 2) && ($days <= 0 || !in_array($days, $valid_days))) {
				$this->error['warning_subs'] = $this->language->get('error_subscription_required');
			}
			 
		} else {
			$this->error['warning_subs'] = $this->language->get('Choose subscription type');
		}		
