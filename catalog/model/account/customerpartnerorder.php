...
class ModelAccountCustomerpartnerOrder extends Model
{
	...
	public function customerpartner($order_info, $order_status, $comment = '', $order_status_id = 0)
	{
		...
		foreach ($order_product_query->rows as $product) {
			...
			if (isset($mpSellers['email']) and !empty($mpSellers['email'])) {
				...
				if ($this->config->get('marketplace_commission_tax')) {
					$product_tax = $this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0;
				}
		...
		if ($this->config->get('marketplace_mailtoseller')) {
			...
			$data['link'] = $this->url->link('account/customerpartner/orderinfo','&order_id='.$order_id);
			...
			foreach ($order_product_query->rows as $order_product) {
				...
				if ($product_download_query->row['total']) {
					$data['download'] = $this->url->link('account/customerpartner/download');
			...
			foreach ($mailToSellers as $value) {
				...
				if ($this->config->get('module_marketplace_status')) {
					...
					if ($SellerIdArray) {
						$this->load->model('account/customerpartner/get_seller_order_mail');
						$seller_details = $this->model_account_customerpartner_get_seller_order_mail->getSellerDataForOrderMail([
							'order_id' => $order_id,
						...
						if ($seller_details) {
							$data['seller_info_status'] = true;
							$return_array['seller_block'] = $this->load->view('account/customerpartner/order_mail_seller_info', $seller_details);
				...
				if (isset($check_order_exists) && $this->config->get('marketplace_mail_order_status_change') && !isset($this->session->data['order_id'])) {
...
if(isset($this->session->data['order_id']) && $this->config->get('marketplace_mail_order')){
	$mailData['mail_id'] = $this->config->get('marketplace_mail_order');
	$commission = $this->model_account_customerpartner->getSellerCommission($value['customer_id']);
