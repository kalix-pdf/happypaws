...
class ModelAccountProductsavedoption extends Model {
	...
	public function getCrosssellProductsOptions($id) {
		$options = $this->db->query("SELECT options, crosssell_id, parent_options, parent_id, child_id,`image`, parent_name as `name` FROM ". DB_PREFIX . "crosssell_related WHERE id ='". $id ."'")->row;
		return $options;
	}

	public function getUpsellProductsOptions($id) {
		$options = $this->db->query("SELECT options, upsell_id, child_id, `image` FROM ". DB_PREFIX . "upsell_related WHERE id ='". $id ."'")->row;
		return $options;
	}
