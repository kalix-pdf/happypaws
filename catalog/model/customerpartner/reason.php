		...
		if (isset($data['reason_description']) && is_array($data['reason_description'])) {
		  foreach ($data['reason_description'] as $key => $reason_description) {
		    if (isset($reason_description['description']) && $reason_description['description']) {
		      $data['reason_description'][$key]['description'] = trim($this->db->escape($reason_description['description']), '\n');
		    }
		  }
		}
...
public function obtainReasons() {
    $sql = "SELECT DISTINCT mp_r.reason_id as id, mprd.title as title 
            FROM " . DB_PREFIX . "mp_reason mp_r 
            LEFT JOIN " . DB_PREFIX . "mp_reason_description mprd ON (mp_r.reason_id = mprd.reason_id) 
            LEFT JOIN " . DB_PREFIX . "customerpartner_to_reason c2r ON (mprd.reason_id=c2r.reason_id) 
            WHERE mprd.language_id = '" . (int)$this->config->get('config_language_id') . "'";

    $sql .= " AND c2r.seller_id IN (0, " . (int)$this->customer->getId() . ")";

    $sql .= " AND mp_r.status = 1 ORDER BY mprd.title";
