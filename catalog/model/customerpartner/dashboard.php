...
class ModelCustomerpartnerDashboard extends Model {
	...
	public function getTotalSales($data = array()) {
		...
		$sql = "SELECT SUM(c2o.customer+c2o.admin) AS total, SUM(c2o.customer) as seller, SUM(c2o.admin) as admin FROM `" . DB_PREFIX . "customerpartner_to_order` c2o LEFT JOIN `".DB_PREFIX ."order` o ON (c2o.order_id = o.order_id) where c2o.customer_id='".(int)$this->customer->getId()."' AND o.order_status_id > 0 ";
	...
	public function getTotalCustomersByMonth() {
		...
		$sql = "SELECT COUNT(DISTINCT o.customer_id) as total, o.date_added FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_status` os ON (o.order_status_id = os.order_status_id) LEFT JOIN `" . DB_PREFIX . "customerpartner_to_order` c2o ON (o.order_id = c2o.order_id) WHERE o.order_status_id IN(" . implode(",", $implode) . ") AND c2o.customer_id = ".(int)$this->customer->getId()." AND DATE(o.date_added) >= '" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "' GROUP BY DATE(o.date_added)";
		$query = $this->db->query($sql);
	...
	public function getTotalCustomersByYear() {
		...
		$sql = "SELECT COUNT(DISTINCT o.customer_id) as total, o.date_added FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_status` os ON (o.order_status_id = os.order_status_id) LEFT JOIN `" . DB_PREFIX . "customerpartner_to_order` c2o ON (o.order_id = c2o.order_id) WHERE c2o.customer_id = ".(int)$this->customer->getId()." AND o.order_status_id IN(" . implode(",", $implode) . ") AND YEAR(o.date_added) = YEAR(NOW()) GROUP BY MONTH(o.date_added)";
		$query = $this->db->query($sql);
