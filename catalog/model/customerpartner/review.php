...
class ModelCustomerpartnerReview extends Model {
   ...
   $sql = "SELECT CONCAT(c.firstname, ' ', c.lastname) AS name,c.customer_id AS customer_id FROM " . DB_PREFIX . "customer c WHERE 1";
	...
	public function approvReview($id) {
		try {

			$this->db->query("UPDATE `" . DB_PREFIX . "customerpartner_to_feedback` SET `status` = 1 WHERE seller_id = '".$this->customer->getId()."' AND id = ".(int)$id);
			return true;
