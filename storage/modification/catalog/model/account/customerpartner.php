...
class ModelAccountCustomerpartner extends Model
{
	...
	public function getProduct($product_id)
	{
		...
		if ($query->num_rows) {
			...
			$this->log->write(['if' => 'inside if']);


		} else {

			$this->log->write(['else' => 'inside else']);
			return false;
		}
	}
	...
	public function getTotalProductsSeller($data = array())
	{
		...
		$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.date_available <= NOW() AND p.status = 1 ";
	...
	public function addProduct($data)
	{
		...
		*/
		...
		if ($this->config->get('module_wk_seller_group_status')) {
			...
			if (isset($data['price'])) {
				$sql .= ", price = '" . (float)$this->db->escape($this->currency->convert((float)$data['price'], $this->session->data['currency'], $this->config->get('config_currency'))) . "'";
			}
			...
			if (isset($data['price'])) {
				$sql .= ", price = '" . (float)$this->db->escape($this->currency->convert((float)$data['price'], $this->session->data['currency'], $this->config->get('config_currency'))) . "',seller_price = '" . (float)$this->db->escape($data['price']) . "'";
			}
	...
	public function editProduct($data)
	{
		...
		if ($this->config->get('module_wk_seller_group_status')) {
			...
			if (isset($data['quantity']) || isset($data['price'])) {
				...
				if (isset($data['price'])) {
					$sqlarray[] = "price = '" . (float)$this->db->escape($this->currency->convert((float)$data['price'], $this->session->data['currency'], $this->config->get('config_currency'))) . "',seller_price = '" . (float)$this->db->escape($data['price']) . "',currency_code = '" . $this->session->data['currency'] . "'";
				}
	...
	public function productQuery($sql, $data)
	{
		...
		if (isset($data['price'])) {
			$implode[] = "price = '" . $this->db->escape($this->currency->convert((float)$data['price'], $this->session->data['currency'], $this->config->get('config_currency'))) . "'";
		}
	...
	public function addOption($data)
	{
		...
		$data['required'] = 1;
		$this->log->write($data['option_value']);
				...
				// 			$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', 
				// 				option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', 
				...
				// 	}
				// } else {
				// 	$this->db->query("INSERT INTO " . DB_PREFIX . "product_option SET product_id = '" . (int)$product_id . "', option_id = '" . (int)$option_id . "', value = '" . $this->db->escape($product_option['value']) . "', required = '" . (int)$product_option['required'] . "'");
				// }
				...
				if (isset($product_option['type']) && ($product_option['type'] == 'select' || $product_option['type'] == 'radio' || $product_option['type'] == 'checkbox' || $product_option['type'] == 'image')) {
					...
					if (isset($product_option['product_option_value']) && count($product_option['product_option_value']) > 0) {
						foreach ($product_option['product_option_value'] as $product_option_value) {
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_option_value SET product_option_id = '" . (int)$product_option_id . "', product_id = '" . (int)$product_id . "', option_id = '" . (int)$product_option['option_id'] . "', option_value_id = '" . (int)$product_option_value['option_value_id'] . "', quantity = '" . (int)$product_option_value['quantity'] . "', subtract = '" . (int)$product_option_value['subtract'] . "', price = '" . (float)$this->currency->convert((float)$product_option_value['price'], $this->session->data['currency'], $this->config->get('config_currency')) . "', price_prefix = '" . $this->db->escape($product_option_value['price_prefix']) . "', points = '" . (int)$product_option_value['points'] . "', points_prefix = '" . $this->db->escape($product_option_value['points_prefix']) . "', weight = '" . (float)$product_option_value['weight'] . "', weight_prefix = '" . $this->db->escape($product_option_value['weight_prefix']) . "'");
						}
					} else {
		...
		if (isset($data['keyword']) and $data['keyword']) {
			...
			foreach ($data['keyword'] as $language_id => $keyword) {
				if (trim($keyword)) {
					...
					if ($this->config->get('module_marketplace_status') && $this->config->get('marketplace_product_seo_product_name')) {

						foreach ($data['product_description'] as $language_id => $value) {

							$this->db->query("INSERT INTO " . DB_PREFIX . "seo_url SET store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$language_id . "', query = 'product_id=" . (int)$product_id . "', keyword = '" . preg_replace('/[\s-]+/', '-', preg_replace('/[^A-Za-z0-9_-]/', ' ', $this->db->escape($value['name']))) . "'");
						}
					}
				}
			}
	...
	public function becomePartner($shop, $country_id, $customer_id, $message = '')
	{
		...
		if ($this->config->get('marketplace_partnerapprov')) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "customerpartner_to_customer set customer_id = '" . (int)$customer_id . "', is_partner='1', country = '" . $this->db->escape($countryCode) . "', commission = '" . 
			(float)$commission . "', companyname = '" . $this->db->escape($shop) . "', screenname = '" . $this->db->escape($shop) . "', countrylogo = '" . $this->db->escape($countryCodeFlag) . "'");
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "customerpartner_to_customer set customer_id = '" . (int)$customer_id . "', is_partner='0', country = '" . 
			$this->db->escape($countryCode) . "', countrylogo = '" . $this->db->escape($countryCodeFlag) . "', companyname = '" . $this->db->escape($shop) . "', screenname = '" . $this->db->escape($shop) . "'");
			
		}

		$this->load->library('b2b');

		$this->b2b->delete('seo_url', ['query'	=> 'seller_id=' . (int)$this->customer->getId()]);

		foreach ($this->b2b->select('language', array('*'), array(), 'rows') as $language_id => $language) {

			$this->b2b->replace('seo_url', [
				'query'	=> 'seller_id=' . (int)$this->customer->getId(),
				'keyword'	=> preg_replace('/[\s-]+/', '-', preg_replace('/[^A-Za-z0-9_-]/', '', $this->db->escape($shop))),
				'store_id'	=> 0,
	...
	public function partnerStore($customer_id, $store_id){
		$this->db->query("INSERT INTO " . DB_PREFIX . "customerpartner_to_store set customer_id = '" . (int)$customer_id . "', store_id='". $store_id ."' ");
	}
	/**
	...
	public function updateProfile($data)
	{
		...
		if (isset($data['youtube']))
			$impolde[] = 'youtube = "' . $this->db->escape($data['youtube']) . '"';

		if (isset($data['instagram']))
			$impolde[] = 'instagram = "' . $this->db->escape($data['instagram']) . '"';
	 ...
	 */
	public function getProfile($customer_id = 0)
	{
		if(!$customer_id) {
	...
	public function addOrderHistory($order_id, $data, $seller_change_order_status_name = '')
	{
		...
		$this->load->model('account/notification');
		$activity_data = array(
			'id' => isset($order_info['customer_id']) ?? 0,
			'status'        => $data['order_status_id'],
			'order_id'    => $order_id
		);
		...
		$subject = sprintf($this->language->get('m_text_subject'), isset($order_info['store_name']) ?? 'store_name', $order_id);

		$message  = $this->language->get('m_text_order') . ' ' . $order_id . "\n";
		$message .= $this->language->get('m_text_date_added') . ' ' . date($this->language->get('m_date_format_short'), strtotime(isset($order_info['date_added'])??'')) . "\n\n";

		$order_status_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$data['order_status_id'] . "' AND language_id = '" . (int)isset($order_info['language_id']) . "'");
		...
		if (version_compare(VERSION, '2.0.1.1', '<=')) {
			...
			/*Old mail code*/
			$mail = new Mail($this->config->get('config_mail'));
			$mail->setTo(isset($order_info['email']));
			$mail->setFrom($sellerEmail);
			$mail->setSender(isset($order_info['store_name']) ?? 'store_name');
			$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
			$mail->setText(html_entity_decode($message, ENT_QUOTES, 'UTF-8'));
			$mail->send();
		} else {
	...
	public function getSellerOrderProducts($order_id)
	{
		...
		$sql = $this->db->query("SELECT op.*,c2o.price price, c2o.price c2oprice, c2o.shipping_applied,c2o.paid_status,c2o.order_product_status FROM " . DB_PREFIX . "customerpartner_to_order c2o LEFT JOIN " . DB_PREFIX . "order_product op ON (c2o.order_product_id = op.order_product_id AND c2o.order_id = op.order_id) WHERE c2o.order_id = '" . (int)$order_id . "'  AND c2o.customer_id = '" . (int)$this->customer->getId() . "' ORDER BY op.product_id ");
...
/**
	 ...
	 */
	public function getorderstatusbyorder($order_id)
	{
		$query = $this->db->query("SELECT order_status_id FROM " . DB_PREFIX . "order 
		WHERE order_id = '" . (int)$order_id . "'");

		return $query->row['order_status_id'] ?? null;
	}
	...
	public function getOrderTotalsForCustomer($order_id, $product_id)
	{
	 ...
	 * @return [type]                [false]
	 */
	public function addsellerorderproductstatus($order_id, $orderstatusid, $parcelNumber)
	{
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_order SET order_product_status = '" . $orderstatusid . "' WHERE order_id = '" . (int)$order_id . "'");	
		$this->db->query("UPDATE " . DB_PREFIX . "order 
        SET order_status_id = '" . (int)$orderstatusid . "' 
        WHERE order_id = '" . (int)$order_id . "'");

		$this->db->query("UPDATE " . DB_PREFIX . "order 
		SET tracking_number = '" . (string)$parcelNumber . "' 
		WHERE order_id = '" . (int)$order_id . "'");
			
    	return true;
	}

	public function changetocancel($cancelOrder, $parcelnum)
	{
		$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_order 
		SET order_product_status = '" . $cancelOrder . "' WHERE tracking_number = '" . (string)$parcelnum . "'");	

		$this->db->query("UPDATE " . DB_PREFIX . "order 
		SET order_status_id = '" . (int)$cancelOrder . "'
		WHERE tracking_number = '" . (string)$parcelnum . "'");	
	}

	public function changetocancelFromPendingStatus($order_id, $cancel_order)
	{
		$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_order 
		SET order_product_status = '" . $cancel_order . "' WHERE order_id = '" . (int)$order_id . "'");	

		$this->db->query("UPDATE " . DB_PREFIX . "order 
		SET order_status_id = '" . (int)$cancel_order . "'
		WHERE order_id = '" . (int)$order_id . "'");	
	}

	public function changetoPickup($tracking_num)
	{
		$pickUp = 17;

		$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_order 
		SET order_product_status = '" . $pickUp . "' WHERE tracking_number = '" . (string)$tracking_num . "'");	

		$this->db->query("UPDATE " . DB_PREFIX . "order 
		SET order_status_id = '" . (int)$pickUp . "'
		WHERE tracking_number = '" . (string)$tracking_num . "'");	
	}
	
	public function changetoInTransit($tracking_num)
	{
		$InTransit = 18;

		$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_order 
		SET order_product_status = '" . $InTransit . "' WHERE tracking_number = '" . (string)$tracking_num . "'");	

		$this->db->query("UPDATE " . DB_PREFIX . "order 
		SET order_status_id = '" . (int)$InTransit . "'
		WHERE tracking_number = '" . (string)$tracking_num . "'");	
	}
	
	public function changetoDelivered($tracking_num)
	{
		$Delivered = 19;

		$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_order 
		SET order_product_status = '" . $Delivered . "' WHERE tracking_number = '" . (string)$tracking_num . "'");	

		$this->db->query("UPDATE " . DB_PREFIX . "order 
		SET order_status_id = '" . (int)$Delivered . "'
		WHERE tracking_number = '" . (string)$tracking_num . "'");	
	}
	...
	public function changetoRate($order_id)
	{
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customerpartner_to_order 
		SET order_product_status = '" . $ToRate . "' WHERE order_id = '" . (int)$order_id . "'");	

		$this->db->query("UPDATE " . DB_PREFIX . "order 
		SET order_status_id = '" . (int)$ToRate . "'
		WHERE order_id = '" . (int)$order_id . "'");	
	}

	public function getOrderStatus($order_id)
	{
		$query = $this->db->query("SELECT order_product_status FROM " . DB_PREFIX . "customerpartner_to_order WHERE order_id = '" . (int)$order_id ."'");
	
		return array_column($query->rows, 'order_product_status');
	}
	 ...
	 * @return [type]             [array]
	 */
	public function getOrderDetails($data = array(), $order_id = 0, $product_id = 0)
	{
	 ...
	 * @return [type]             [array]
	 */
	public function getOrderDetailsTotal($data = array(), $order_id = 0, $product_id = 0)
	{
	...
	/**
	 * [getCustomersLists to get orders according to sellers]
	 * @param  [array]  $data [filter keywords]
	 * @return [array]       [order details]
	 */
	public function getCustomersLists($data = array())
	{

		$sql1 = "SELECT DISTINCT o.customer_id FROM `" . DB_PREFIX . "order_product` op LEFT JOIN " . DB_PREFIX . "customerpartner_to_order c2o ON (op.product_id = c2o.product_id AND op.order_id = c2o.order_id) LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) where c2o.customer_id='" . (int)$this->customer->getId() . "' AND o.order_status_id > 0 ";


		$customers = $this->db->query($sql1)->rows;

		if (!empty($customers)) {
			$sql = "SELECT DISTINCT SUM(o.total) as total, o.currency_code, o.customer_id, o.telephone, o.email, CONCAT(o.firstname,' ',o.lastname) as name, oc.date_added
			FROM `" . DB_PREFIX . "order` o";
			$sql .= " LEFT JOIN " . DB_PREFIX . "customerpartner_to_order oc ON (oc.order_id = o.order_id) WHERE o.customer_id IN (" . implode(',', array_column($customers, 'customer_id')) . ") AND o.order_status_id != '" . $this->config->get('marketplace_cancel_order_status') . "'";
	...
	/**
	 * [getCustomersListsTotal total count of orders of particular seller]
	 * @param  [array]  $data [filter keywords]
	 * @return [integer]       [total number of orders]
	 */
	public function getCustomersListsTotal($data = array())
	{
		...
		$customers = $this->db->query($sql1)->rows;
		if (!empty($customers)) {
			$sql = "SELECT SUM(oc.customer) as total, oc.customer_id, o.telephone, o.email, CONCAT(o.firstname,' ',o.lastname) as name, oc.date_added
			FROM `" . DB_PREFIX . "order` o";
			$sql .= " LEFT JOIN " . DB_PREFIX . "customerpartner_to_order oc ON (oc.order_id = o.order_id) WHERE o.customer_id IN (" . implode(',', array_column($customers, 'customer_id')) . ") AND o.order_status_id != '" . $this->config->get('marketplace_cancel_order_status') . "'";
