...
class ControllerProductSearch extends Controller {
	public function index() {
        ...
        /**
          ...
          */
          if ($this->config->get('module_marketplace_status')) {
            $check_seller_product = $this->db->query("SELECT * FROM " . DB_PREFIX . "customerpartner_to_product WHERE product_id = '" . (int)$result['product_id'] . "'")->row;
            ...
            if ($check_seller_product) {
              ...
              if ($this->config->get('marketplace_commission_tax')) {
                ...
                $commission_array = $this->model_account_customerpartnerorder->calculateCommission(array('product_id' => $result['product_id'], 'product_total' => $this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax'))), $check_seller_product['customer_id']);
              } else {
                $commission_array = $this->model_account_customerpartnerorder->calculateCommission(array('product_id' => $result['product_id'], 'product_total' => $result['price']), $check_seller_product['customer_id']);
              }
        ...
        $price =  $this->currency->format($result['commission_amount'] + $this->tax->calculate($result['price']- $result['commission_amount'], $result['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
