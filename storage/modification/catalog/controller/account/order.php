	...
	public function info() {
		...
		if ($order_info) {
			...
			$products = $this->model_account_order->getOrderProducts($this->request->get['order_id']);
                              ...
                              $data['checkIsSellerProduct'] = true;
			                        $this->load->library('b2b');

                              $data['sold_invoice'] = $this->url->link('account/soldinvoice', 'order_id=' . $order_info['order_id'], true);
				...
				foreach ($options as $option) {
					...
					$option_data[] = array(
						...
						'value' => (utf8_strlen($value) > 20 ? utf8_substr($value, 0, 20) . '..' : $value)
                          ...
                          $sellerInfo = $this->b2b->select('customerpartner_to_product',array('customer_id'),array('product_id'=>$product['product_id']),'row');
                          $sellerId = 0;
                          if(!empty($sellerInfo)) {
                              $sellerId = $sellerInfo['customer_id'];
                            $data['checkIsSellerProduct'] = false;
                          }
            ...
            ,
					...
					'totals'   => $this->currency->format($product['price'] * $product['quantity'], $order_info['currency_code'], $order_info['currency_value']),
					'reorder'  => $reorder,
                          ...
                          'order_detail'   => $this->url->link('account/customerpartner/order_detail', 'order_id=' . $order_info['order_id'] . '&product_id=' . $product['product_id'], true),
                            // 'order_invoice'   => $this->url->link('account/soldinvoice', 'order_id=' . $order_info['order_id'] . '&product_id=' . $product['product_id'] . '&seller_id=' . $sellerId, true),
			...
			$data['logs'] = $this->load->controller('extension/displaywebhook', $order_info['tracking_number']);
