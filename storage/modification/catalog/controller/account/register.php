	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if($this->config->get('module_marketplace_status')){
              ...
              $this->load->model('account/customerpartner');
              $this->model_account_customerpartner->partnerStore($customer_id,$this->config->get('config_store_id') );
              }
		...
		if (isset($this->request->post['firstname']) && trim($this->request->post['firstname']) != '') {
			$data['firstname'] = trim($this->request->post['firstname']);
		} else {
			$data['firstname'] = '';
		}

		if (isset($this->request->post['lastname']) && trim( $this->request->post['lastname']) != '') {
		...
		if (isset($this->request->post['password']) && trim( $this->request->post['password']) != 'password') {
        ...
        if($this->config->get('module_marketplace_status') && $this->config->get('marketplace_becomepartnerregistration') AND isset($this->request->post['tobecomepartner'])){
            ...
            $this->load->language('account/customerpartner/become_partner');
              ...
              if(utf8_strlen($this->request->post['shoppartner'])<=3 && $this->request->post['tobecomepartner']==1){
                ...
                $this->error['error_shoppartner'] = $this->language->get('error_noshop');
                }
              }
