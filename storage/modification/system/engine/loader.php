...
final class Loader {
	...
	public function view($route, $data = array()) {
		...
		if ($result && !$result instanceof Exception) {
          ...
          if ($this->registry->get('config')->get('marketplace_separate_view') && isset($this->session->data['marketplace_separate_view']) && $this->session->data['marketplace_separate_view'] == 'separate' && preg_match('/account\/customerpartner/',$route) && isset(explode('/',$route,2)[1])) {
            $output = $template->render('default/template/' . $route);
          } elseif ($this->registry->get('config')->get('marketplace_separate_view') && isset($this->session->data['marketplace_separate_view']) && $this->session->data['marketplace_separate_view'] == 'separate' && preg_match('/common\/filemanager/',$route) && isset(explode('/',$route,2)[1])) {
            $output = $template->render('default/template/common/filemanager');
          } else {
