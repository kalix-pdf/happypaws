              ...
              if($product) {
                ...
                if($commission)
                  ...
                  if($this->config->get('module_wk_seller_group_status')) {
                    ...
                    $result = end($this->db->query("SELECT * FROM ".DB_PREFIX."seller_group_customer WHERE customer_id = '" . $customer_id."'")->rows);
                    if($result && !$result['suspended']) {
                      ...
                      foreach ($categories as $category) {
                        if ($category && isset($category['category_id']) && $category['category_id']) {
                          $groupCommission = $this->db->query("SELECT percentage,fixed FROM ".DB_PREFIX."seller_group_commission_categorywise WHERE category_id = '" . $category['category_id'] . "' AND seller_id = '" . $customer_id . "' AND group_id = '" . $result['gid']."' ")->row;
                          if (empty($groupCommission)) {
                            $groupCommission = $this->db->query("SELECT customerDefaultGroupCommissionPercentage as percentage,customerDefaultGroupCommissionFixed as fixed FROM ".DB_PREFIX."seller_group_customer WHERE customer_id = '" . $customer_id."' AND gid = '" . $result['gid'] . "' ")->row;
                          }
                        } else {
                          $groupCommission = $this->db->query("SELECT customerDefaultGroupCommissionPercentage as percentage,customerDefaultGroupCommissionFixed as fixed FROM ".DB_PREFIX."seller_group_customer WHERE customer_id = '" . $customer_id."' AND gid = '" .  $result['gid'] ."' ")->row;
                        }
                      }
                     	if(isset($groupCommission) && $groupCommission) {
                        $commission_amount += ( $groupCommission['percentage'] ? ($groupCommission['percentage']*$product['product_total']/100) : 0 ) + $groupCommission['fixed'];
                      }
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
					$option_query = $this->db->query("
						...
						LEFT JOIN seller_product_option so ON (po.option_id = so.seller_option_id) 
						WHERE po.option_id = '" . (int)$product_option_id . "' 
					...
					if ($option_query->num_rows) {
						if ($option_query->row['type'] == 'select') {
							$option_value_query = $this->db->query("
								...
								LEFT JOIN seller_product_option_value ov ON (pov.option_value_id = ov.option_value_id) 
								WHERE pov.option_value_id = '" . (int)$value . "' 
								AND pov.option_id = '" . (int)$product_option_id . "'
							");
							
							// if (!empty($option_value_query->num_rows)) {
							// 	error_log(print_r($option_value_query->num_rows, true));
								
							// } else {
							// 	error_log('No rows returned in $option_query');
							// }
							...
							if ($option_value_query->num_rows) {
								...
								if ($option_value_query->row['subtract'] && 
									...
									$option_value_query->row['quantity'] < $cart['quantity'])) {
									$stock = false;
								}
								$option_data[] = array(
									...
									'value'                   => $option_value_query->row['value'],
