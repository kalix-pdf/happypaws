...
class ModelCustomerpartnerDashboard extends Model {
	...
	public function getTotalSales($customer_id,$data = array()) {
		$sql = "SELECT SUM(c2o.customer+c2o.admin) AS total, SUM(c2o.customer) as seller, SUM(c2o.admin) as admin FROM `" . DB_PREFIX . "customerpartner_to_order` c2o LEFT JOIN `".DB_PREFIX ."order` o ON (c2o.order_id = o.order_id) where c2o.customer_id='".(int)$customer_id."' AND o.order_status_id > 0 ";
	...
	// customer
	public function getTotalCustomers($customer_id,$data = array()) {
	...
	public function getTotalOrdersByMonth($customer_id) {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, o.date_added FROM `" . DB_PREFIX . "order_product` op LEFT JOIN ".DB_PREFIX ."customerpartner_to_order c2o ON (op.product_id = c2o.product_id AND op.order_id = c2o.order_id) LEFT JOIN `".DB_PREFIX ."order` o ON (op.order_id = o.order_id) WHERE c2o.customer_id='".$customer_id."' AND o.order_status_id IN(" . implode(",", $implode) . ") AND DATE(o.date_added) >= DATE('" . $this->db->escape(date('Y') . '-' . date('m') . '-1') . "') GROUP BY DATE(o.date_added)");
	...
	public function getTotalOrdersByYear($customer_id) {
		...
		$query = $this->db->query("SELECT COUNT(*) AS total, o.date_added FROM `" . DB_PREFIX . "order_product` op LEFT JOIN ".DB_PREFIX ."customerpartner_to_order c2o ON (op.product_id = c2o.product_id AND op.order_id = c2o.order_id) LEFT JOIN `".DB_PREFIX ."order` o ON (op.order_id = o.order_id) WHERE c2o.customer_id='".$customer_id."' AND o.order_status_id IN(" . implode(",", $implode) . ") AND YEAR(o.date_added) = YEAR(NOW()) GROUP BY MONTH(o.date_added)");
