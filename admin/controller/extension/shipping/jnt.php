...
class ControllerExtensionShippingjnt extends Controller{
	...
	public function index () {
		...
		if (isset($this->request->post['shipping_jnt_status'])) {
	      	$data['shipping_jnt_status'] = $this->request->post['shipping_jnt_status'];
	    } else {
	      	$data['shipping_jnt_status'] = $this->config->get('shipping_jnt_status');
	    }

	     if (isset($this->request->post['shipping_jnt_tax_class_id'])) {
	      	$data['shipping_jnt_tax_class_id'] = $this->request->post['shipping_jnt_tax_class_id'];
	    } else {
	      	$data['shipping_jnt_tax_class_id'] = $this->config->get('shipping_jnt_tax_class_id');
	    }

	    if (isset($this->request->post['shipping_jnt_geo_zone_id'])) {
	      	$data['shipping_jnt_geo_zone_id'] = $this->request->post['shipping_jnt_geo_zone_id'];
	    } else {
	      	$data['shipping_jnt_geo_zone_id'] = $this->config->get('shipping_jnt_geo_zone_id');
	    }

	    if (isset($this->request->post['shipping_jnt_sort_order'])) {
			$data['shipping_jnt_sort_order'] = $this->request->post['shipping_jnt_sort_order'];
		} else {
			$data['shipping_jnt_sort_order'] = $this->config->get('shipping_jnt_sort_order');
		}
